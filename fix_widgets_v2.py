...
<script>
fetch('https://api.rss2json.com/v1/api.json?rss_url=https://www.trthaber.com/manset_articles.rss')
    ...
    for file in files:
        if file.endswith(('.html', '.htm', '.php', '.tpl')):
            ...
            try:
                ...
                # Update TRT script to use new RSS feed
                if 'fetch(\'https://api.rss2json.com/v1/api.json?rss_url=https%3A%2F%2Fwww.trthaber.com%2Fxml_mobile.php%3Ftur%3Dxml_genel\')' in content:
                ...
                # Fix Facebook API version from v2.5 to v20.0
                if 'version=v2.5' in content:
                    content = content.replace('version=v2.5', 'version=v20.0')
                    changed = True
