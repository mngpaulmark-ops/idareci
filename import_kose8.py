...
for part in parts[1:]:
    ...
    stmt = part[:end_idx]
    # stmt should be like:  (`id`, `head`, ...) VALUES (1, '...', ...), (2, '...', ...)
    val_idx = stmt.find('VALUES')
    if val_idx == -1: continue
    
    val_str = stmt[val_idx+6:].strip()
    
    # We have a string like: (1, 'a', 'b'), (2, 'c', 'd')
    # Let's parse it character by character cleanly
    tuples = []
    current_tuple = []
    current_val = ""
    in_string = False
    ...
    for cols in tuples:
        if len(cols) >= 11:
            try:
                ...
                info = cols[7].strip("'").replace("\\r\\n", "\n").replace("\\n", "\n").replace("\\'", "'")
                gid = int(cols[9].strip())
